    ...
    public class ClientController : Controller
    {
        ...
        public async Task<JsonResult> Create(ClientCreateView model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    _logger.Error(ex, $"{DateTime.Now.ToLongTimeString()} : Server error, when try add client!");
        ...
        public async Task<PartialViewResult> Edit(int id)
        {
            if (id > 0)
            {
                ...
                {
                    _logger.Error(ex, $"{DateTime.Now.ToLongDateString()} : Server error, when trying to get client by Id ({id})!");
        ...
        public async Task<JsonResult> Edit(ClientEditView model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    _logger.Error(ex, $"{DateTime.Now.ToLongTimeString()} : Server error, when trying to edit a client!");
        ...
        public async Task<JsonResult> Delete(int id)
        {
            if (id > 0)
            {
                ...
                {
                    _logger.Error(ex, $"{DateTime.Now.ToLongTimeString()} : Server error, when trying to delete a client!");
